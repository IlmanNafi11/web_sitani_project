                ...
                <div class="main-content w-full overflow-y-auto p-6 rounded-xl bg-white border border-[#E5E8EC] h-fit">
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="form-container">
                            <form id="form-update-desa" action="{{ route('desa.update', $desa->id) }}" method="post">
                                @method('PUT')
                                @csrf
                                    ...
                                    <div class="flex flex-col gap-3 mb-5">
                                        ...
                                        <select name="kecamatan_id" id="kecamatan_id"
                                            ...
                                            @foreach ($kecamatans as $kecamatan)
                                                <option value="{{ $kecamatan->id }}" {{ $kecamatan->id == $desa->kecamatan_id ? 'selected': '' }}>
                                                    {{ $kecamatan->nama }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <x-ui.button-save formId="form-update-desa" />
                            </form>
                        </div>
                    </div>
                </div>
