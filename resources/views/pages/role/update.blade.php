...
@section('title', 'Update Role | Sitani')
@section('content')
    <x-ui.card>
        <x-ui.result-alert/>

        <div class="mb-5 border-b border-[#E5E8EC]">
            <x-ui.title :title="'Update Role'"/>
        </div>
        <form id="form-update-role" action="{{ route('admin.roles.update', $role->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <x-form.input-text :keyId="'name'" :label="'Nama Role'" :name="'name'"
                    :placeholder="'Masukkan nama role'" :defaultValue="$role->name" />
