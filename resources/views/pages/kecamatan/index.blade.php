            ...
            <div class="content flex-1 w-full p-8 flex">
                <div class="main-content w-full overflow-y-auto p-6 rounded-xl bg-white border border-[#E5E8EC]">
                    <div class="flex mb-8 flex-col gap-4">
                        <div>
                            <span class="title-label-page">Data Kecamatan</span>
                        </div>
                        <div>
                            <x-ui.bread-crumb :breadcrumbs="[['name' => 'Kecamatan', 'url' => route('kecamatan.index')], ['name' => 'Data Kecamatan']]" />
                        </div>
                    </div>
                    <table id="kecamatan-table" class="table">
                        <thead>
                            <tr>
                                <th class="bg-[#F6F8FB] border-b border-[#E5E8EC] grow">
                        ...
                        <tbody>
                            @forelse ($kecamatans as $Kecamatan)
                                <tr class="text-black">
                                    <td class="grow">{{ $Kecamatan->nama }}</td>
                                    <td class="flex gap-3 w-full">
                                        <x-ui.button-edit route="kecamatan.edit" id="{{ $Kecamatan->id }}" />
                                        <x-ui.button-delete route="kecamatan.destroy" id="{{ $Kecamatan->id }}" />
                                    </td>
                                </tr>
                            @empty
