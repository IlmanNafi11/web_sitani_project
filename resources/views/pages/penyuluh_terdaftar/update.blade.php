                ...
                <div class="main-content w-full overflow-y-auto p-6 rounded-xl bg-white border border-[#E5E8EC] h-fit">
                    <div class="flex flex-col gap-4">
                        <div class="border-b border-[#E5E8EC]">
                            <span class="title-label-page font-medium mb-4 block">Perbarui Data Penyuluh</span>
                        </div>
                        <div class="form-container">
                            <form id="form-update-penyuluh" action="{{ route('penyuluh-terdaftar.update', $penyuluh->id) }}" method="post">
                                @method('PUT')
                                @csrf
                                <div class="w-full grid grid-cols-1 md:grid-cols-2 grid-rows-3 md:grid-rows-2 gap-4 mb-4 md:mb-0">
                                    ...
                                    <div class="flex flex-col gap-3 col-span-1 row-span-1">
                                        ...
                                        <input
                                            ...
                                            value="{{ old('alamat', $penyuluh->alamat) }}">
                                        @error('alamat')
                                            <p class="text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="flex flex-col gap-3">
                                        ...
                                        <select name="kecamatan_id" id="kecamatan_id"
                                            ...
                                            @foreach ($kecamatans as $kecamatan)
                                                <option value="{{ $kecamatan->id }}" {{ $kecamatan->id == $penyuluh->kecamatan_id ? 'selected' : ''}}>
                                                    {{ $kecamatan->nama }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <x-ui.button-save formId="form-update-penyuluh" />
                            </form>
                        </div>
                    </div>
                </div>
